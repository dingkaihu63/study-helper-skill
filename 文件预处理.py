...
class ScanNotSupportedException(Exception):
    """扫描版PDF不支持异常"""
    pass
...
class TextSplitter:
    ...
    def _find_fallback_split_point(self, text: str, target_pos: int) -> int:
        """
        回退寻找切割点：从 target_pos 往前找，优先找 \n\n，其次找中文句号。
        确保不会把一句话切成两半。
        """
        min_pos = max(0, target_pos - 200)  # 最多回退200字符

        # 优先找段落换行 \n\n
        search_range = text[min_pos:target_pos]
        para_break = search_range.rfind('\n\n')
        if para_break != -1:
            return min_pos + para_break + 2  # 切在换行之后

        # 其次找中文句号
        period_pos = search_range.rfind('。')
        if period_pos != -1:
            return min_pos + period_pos + 1  # 切在句号之后

        # 再次找英文句号+空格
        eng_period_pos = search_range.rfind('. ')
        if eng_period_pos != -1:
            return min_pos + eng_period_pos + 2

        # 兜底：找空格或逗号
        space_pos = search_range.rfind(' ')
        if space_pos != -1:
            return min_pos + space_pos + 1

        comma_pos = search_range.rfind('，')
        if comma_pos != -1:
            return min_pos + comma_pos + 1
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        for separator in self.separators:
            if separator == "":
                ...
                while start < len(text):
                    ...
                    # 回退寻找合适的切割点
                    split_pos = self._find_fallback_split_point(text, end)
                    # 确保至少前进一定距离，避免死循环
                    if split_pos <= start:
                        split_pos = end
                    ...
                    if start <= split_pos - self.chunk_size + 1:
                        start = split_pos
        ...
        # 最终兜底：回退寻找切割点
        split_pos = self._find_fallback_split_point(text, self.chunk_size)
        first_chunk = text[:split_pos]
        remaining = text[split_pos:]
        result = [first_chunk.strip()] if first_chunk.strip() else []
        if remaining.strip():
            result.extend(self.split_text(remaining))
        return result
...
class DocumentProcessor:
    ...
    def process_pdf(self, file_path: str) -> ProcessedDocument:
        ...
        for page_num, page in enumerate(doc, 1):
            ...
            for block in blocks:
                ...
                if len(text) > self.splitter.chunk_size:
                    ...
                    for sub_text in sub_chunks:
                        chunk_metadata = {"bbox": [x0, y0, x1, y1], "block_no": block_no}
                        if page_is_scanned:
                            chunk_metadata["is_scanned"] = True
            ...
            # 扫描版PDF检测：如果一页的有效字符数少于50个中文字符，标记为扫描页
            combined_page_text = "\n".join(page_texts)
            meaningful_chars = self._count_meaningful_chars(combined_page_text)
            if meaningful_chars < 50:
                scanned_pages += 1
                page_is_scanned = True
                # 给该页已创建的 chunk 补充标记
                for chunk in chunks:
                    if chunk.page_num == page_num and "is_scanned" not in chunk.metadata:
                        chunk.metadata["is_scanned"] = True
        ...
        # 如果80%以上页面被标记为扫描页，抛出异常
        if total_pages > 0 and scanned_pages / total_pages >= 0.8:
            raise ScanNotSupportedException(
                f"当前文件大概率是扫描版图片（{scanned_pages}/{total_pages} 页无有效文本），"
                f"请先进行 OCR 识别处理后再导入。"
            )

        metadata["scanned_pages"] = scanned_pages
        metadata["scanned_ratio"] = round(scanned_pages / total_pages, 2) if total_pages > 0 else 0

        return self._build_document(file_path, "pdf", total_pages, chunks, metadata, all_text)
    ...
    def process_ppt(self, file_path: str) -> ProcessedDocument:
        ...
        for slide_num, slide in enumerate(prs.slides, 1):
            ...
            for shape in slide.shapes:
                ...
                # 提取表格（表格也作为该页内容的一部分，不单独拆分为独立 chunk）
                if shape.has_table:
                    table = shape.table
                    rows = []
                    for row in table.rows:
                    ...
                    table_text = self._extract_table_text(rows)
                    if table_text:
                        slide_texts.append(f"[表格]\n{table_text}")
                        all_text.append(table_text)

            # 将整页内容合并为一个 chunk，不再进行分块
            if slide_texts:
                # 去重同时保持顺序
                seen = set()
                unique_texts = []
                ...
                combined = "\n".join(unique_texts)
                section_title = slide_title or f"第{slide_num}页幻灯片"
                self._current_section = section_title

                chunks.append(self._create_chunk(
                    combined, ContentType.TEXT.value,
                    {"slide": slide_num, "shape_count": len(slide.shapes)},
                    slide_num
                ))
                # 确保 section_title 被正确设置
                chunks[-1].section_title = section_title
